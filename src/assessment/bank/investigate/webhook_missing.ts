...
function buildA(rng: Rng): VariantWorld {
  ...
  return {
    ...
    q1Options: [
      { text: "השרת של גשר ה-CRM נפל ולא עלה" },
      {
        text: "ה-secret לחתימת ה-webhook הוחלף בצד החנות, והגשר עדיין מאמת עם הישן",
        correct: true,
      },
      { text: "שדרוג ה-Node שבר את ניתוח ה-JSON" },
      { text: "ה-endpoint שגוי בהגדרות החנות" },
      { text: "החנות הפסיקה לשלוח webhooks" },
    ],
    ...
    antiPatterns: genericAntiPatterns({
      irreversible_action: "לעשות rollback לשדרוג ה-Node",
      treat_symptom: "לבקש מהחנות לשלוח מחדש את כל ההזמנות מאתמול",
      fix_decoy: "לבדוק את שדרוג ה-Node ולוודא שהוא לא שבר משהו",
      busywork_gather_more: "להוריד את כל ה-Logs של השבוע ולעבור עליהם לפני שנוגעים במשהו",
    }),
  };
}
...
function buildB(rng: Rng): VariantWorld {
  ...
  return {
    ...
    decisiveArtifactKeyQ3: "log",
    q1Options: [
      { text: "ה-secret לחתימת ה-webhook הוחלף בצד החנות" },
      {
        text: `הפריסה האחרונה שינתה את נתיב ה-webhook הנכנס ל-${newPath}, אבל הגדרת ה-endpoint בצד החנות עדיין מצביעה ל-${oldPath}`,
        correct: true,
      },
      { text: "עלייה ב-CPU של השרת גרמה לדחיית בקשות" },
      { text: "החנות הפסיקה לשלוח webhooks לגמרי" },
    ],
    ...
    isEscalationRequired: false,
    antiPatterns: genericAntiPatterns({
      irreversible_action: "לעשות rollback לפריסה 2.6.0 במלואה",
      treat_symptom: "לבקש מהחנות לשלוח מחדש את כל ההזמנות מאתמול",
      fix_decoy: "לחקור את עליית ה-CPU לפני שממשיכים",
      busywork_gather_more: "לאסוף את כל ה-Deploy notes של החודש האחרון ולנתח אותם",
    }),
  };
}
...
function buildC(rng: Rng): VariantWorld {
  ...
  return {
    ...
    q1Options: [
      { text: "ה-secret לחתימת ה-webhook פג תוקף" },
      { text: "הגדרת ה-endpoint בצד החנות שגויה" },
      {
        text: "מפתח ה-API מול ה-CRM פג תוקף, ולכן הכתיבה ל-CRM נכשלת גם שה-webhook עצמו מתקבל בהצלחה",
        correct: true,
      },
      { text: "רשומת ה-DNS של הדומיין הפנימי השתנתה" },
    ],
    ...
    antiPatterns: genericAntiPatterns({
      irreversible_action: "למחוק את האינטגרציה עם ה-CRM ולהתחיל מחדש",
      treat_symptom: "לבקש מהחנות לשלוח שוב את כל ההזמנות שנכשלו",
      fix_decoy: "לבדוק את רשומות ה-DNS של הדומיין הפנימי",
      busywork_gather_more: "לאסוף היסטוריית מפתחות API של השנה האחרונה לפני שפועלים",
    }),
  };
}
