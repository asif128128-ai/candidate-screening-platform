...
const CASES_EASY: Case[] = [
  {
    ...
    wrong: [
      "polling כל 24 שעות — מספיק כי מדובר בעדכון מלאי שאינו קריטי לזמן אמת ברוב המקרים",
      "ייצוא קובץ ידני פעם בשבוע — נותן תמונה מלאה של המלאי בלי להעמיס על המערכות",
      "polling כל שנייה כדי לא לפספס אף עדכון מלאי, גם אם המערכת תומכת ב-webhook",
    ],
  },
];
...
const CASES_MODERATE: Case[] = [
  {
    ...
    wrong: [
      "webhook — גם אם המערכת הישנה לא תומכת בזה בכלל, כדאי לממש אותו כדי לקבל עדכון מיידי",
      "ייצוא קובץ פעם בחודש — מספיק כי הנתונים ההיסטוריים לא משתנים משמעותית בין ייצוא לייצוא",
      "polling פעם ביום מספיק — התדירות היומית חוסכת עומס על המערכת הישנה ועדיין נותנת תמונה עדכנית יחסית",
    ],
  },
  {
    ...
    wrong: [
      "webhook על כל שינוי — נותן דיוק מרבי בדוח השבועי כי כל עדכון בודד נלכד ברגע שהוא קורה",
      "polling כל דקה — תדירות גבוהה מבטיחה שהדוח השבועי לא יפספס אף שינוי בנתונים ההיסטוריים",
      "polling כל שנייה כדי לוודא עדכניות מלאה של הנתונים ברגע שמכינים את הדוח השבועי",
    ],
  },
  {
    ...
    wrong: [
      "webhook בלבד — מספיק כי רוב האירועים מגיעים, וההשקעה בשכבת polling נוספת אינה מוצדקת",
      "polling כל שעה בלבד, בלי webhook — פשוט יותר לתחזק שכבה אחת ונותן עדיין רזולוציה סבירה",
      "polling כל שנייה כדי לפצות על אמינות נמוכה של ה-webhook, בלי להסתמך עליו כלל",
    ],
  },
];
